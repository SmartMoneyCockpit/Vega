# ---- Vega Command Center (Render-ready) ----
# - Works with or without a /src directory
# - Uses Google Sheet via sheets_client.py
# - Low-API design; optional NewsAPI/Polygon/yfinance
# --------------------------------------------------

# Bootstrap: make imports work regardless of layout (root or /src)
import os, sys
...
# Dependency check (optional, no-op if not present)
try:
    ...
    try:
        ...
        def show_missing():  # no-op
            pass
show_missing()

# Std libs & third-party
import math, json, io, zipfile, time, statistics as stats
...
# Sheets client (supports src/ or root layout)
try:
...
def rows_to_df(rows):
    ...
    data = [
        list(r) + [""] * (len(hdr) - len(r))
        for r in rows[1:] if any(str(x).strip() for x in r)
    ]
...
# Environment (support old/new names)
POLY      = os.getenv("POLYGON_API_KEY") or os.getenv("POLYGON_KEY")
...
# Default fees (overridden by presets)
...
NA_COUNTRIES   = [s.strip() for s in (CFG.get("NA_COUNTRIES", "US,CA,MEX").split(",")) if s.strip()]
...
def is_market_open(region: str) -> bool:
    ...
    if region == "NA":
        ...
        mins = now_utc.hour * 60 + now_utc.minute
        return 0 <= mins <= 360  # APAC early UTC window
...
def save_df(tab: str, df: pd.DataFrame, audit_note: str = None):
...
def match_preset_for(country_code: str, preset_name: str = None):
    ...
    if not df.empty and preset_name:
        ...
        if not z.empty:
            r = z.iloc[0]
            return (
                float(_to_float(r.get("Base", 0))),
                float(_to_float(r.get("BpsBuy", 0))),
                float(_to_float(r.get("BpsSell", 0))),
                float(_to_float(r.get("TaxBps", 0))),
            )
    if not df.empty:
...
def page_risk_lab():
    ...
    st.caption("Quick estimate using last 60 trading days. Guidance only.")
    ...
    for s in syms:
        try:
            t = yf.Ticker(s)
            h = t.history(period="3mo")["Close"].pct_change().dropna()
            if not h.empty:
                prices[s] = h.tail(60)
        except Exception:
            pass
...
# ---------- Options Strategy Builder (robust fetch) ----------
def N(x): 
...
def page_options_builder():
    ...
    sym = st.text_input("Symbol (US/CA)", value="AAPL").strip().upper()

    def _safe_options_expirations(symbol: str, tries: int = 3, pause: float = 0.8):
        for i in range(tries):
            try:
                exps = list((yf.Ticker(symbol).options or [])) if yf else []
                if exps:
                    return exps
            except Exception:
                pass
    ...
    def _safe_option_chain(symbol: str, expiry: str, tries: int = 3, pause: float = 0.8):
        for i in range(tries):
            try:
                chain = yf.Ticker(symbol).option_chain(expiry) if yf else None
                if chain:
    ...
    if not exps:
        st.warning("Options data unavailable for this symbol right now. Try another US/CA ticker or retry.")
    ...
    if calls is None or puts is None:
        st.warning("Could not load option chain (upstream issue). Try another expiration or press Retry.")
...
def page_broker_import():
    ...
    mapping = {}
    if prof != "Custom":
        mapping = dict(PROFILES[prof])
        st.info("Using profile defaults. Adjust below if needed.")
    fields = ["Timestamp", "Symbol", "Side", "Qty", "Price", "Note"]
    cols = list(df.columns)
    for k in fields:
        mapping[k] = st.selectbox(k, ["<none>"] + cols, index=(cols.index(mapping[k]) + 1) if (prof != "Custom" and mapping.get(k) in cols) else 0, key=f"map_{k}")
    if st.button("Append rows"):
        out = []
        for _, r in df.iterrows():
            vals = {k: (r[mapping.get(k)] if mapping.get(k) not in (None, "<none>") and mapping.get(k) in r else "") for k in fields}
            tid = f"{vals.get('Symbol','')}-{int(time.time())}"
            row = [str(vals.get("Timestamp") or now()), tid, str(vals.get("Symbol","")).upper(), str(vals.get("Side","")).upper(), _to_float(vals.get("Qty"),0.0), _to_float(vals.get("Price"),0.0), str(vals.get("Note") or ""), "", "", "", "", "", "", f"{now()}|{APP_VER}|IMPORT"]
            out.append(row)
        for row in out:
            append_trade_log(row, tab_name=log_tab)
...
2. On **Watch List**, add rows with `Ticker, Country, Entry, Stop, Target`.  
3. In **Config** tab, you can set `ALERT_PCT`, `RR_TARGET`, `ACCOUNT_EQUITY`, `RISK_PCT`.

**Fees / Presets** — edit the **Fee_Presets** tab: `Preset, Markets (US,JP,AU,..), Base, BpsBuy, BpsSell, TaxBps`.
In the close box, choose a preset or leave `<auto-match>`.

**Trading**
- Use **Quick Entry** to log trades. `ExitPrice` + `ExitQty` auto-compute `PnL` and `R` per unit.
- Close modes: **Single**, **Average**, **FIFO**, **LIFO**. Fees auto-calc or enter manually.

**Dashboards** — inside each cockpit, open **Positions dashboard** for Realized/Open P/L, Win %, Avg R. Filter by **Tags**.

**Earnings** — press **Sync earnings** to snapshot dates into `Earnings`.

**Risk Lab** — basket VaR estimate (95%, 60d).

**Options** — robust loader; try another ticker/expiry if Yahoo is flaky.

**Broker Import** — map CSV columns or use profiles (IBKR/Tastytrade/Fidelity).

**FX & Hedges** — maintain `FX_Exposure`; click **Optimize** for ETF suggestions.

**Backups** — snapshot tabs or download a CSV bundle.
""")
