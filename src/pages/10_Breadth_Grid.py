...
df = get_breadth_df(); st.dataframe(df, use_container_width=True)
...
if st.button('📸 Save PNG Snapshot'):
    ...
    os.makedirs('snapshots', exist_ok=True); ts=datetime.now().strftime('%Y-%m-%d_%H%M%S')
    path=f'snapshots/breadth_{ts}.png'; fig.savefig(path, bbox_inches='tight', dpi=200); st.success('Saved '+path)
