...
with c3:
    theme = st.selectbox("Theme", ["dark","light"],
                         ...
                         key="theme")
with c4:
    mode = st.selectbox("Mode", ["tvjs","iframe"],
...
ov = st.multiselect("Select studies", choices,
                    default=st.session_state.get("ov", default_sel if mode != "iframe" else []),
                    key="ov")
...
if st.button("Reset to Vega Defaults (HA + EMAs + Built-ins)"):
    st.session_state["symbol"] = "NYSE:RBC"
...
render_chart(symbol, interval=interval, theme=theme, height=600,
             overlays=(codes if mode!='iframe' else None), mode=mode)
